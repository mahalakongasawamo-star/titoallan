...
const shouldReduceMotion = prefersReducedMotion || isMobileScreen;
const animateOnScroll = !shouldReduceMotion;

function showHeroContentInstant() {
...
heroTl.fromTo('.hero__stat-num span',
  { textContent: 0 },
  { textContent: 93, duration: 1.5, ease: 'power2.out', snap: { textContent: 1 } },
  '-=0.8'
);
...
// Fallback: force-hide after 2s even if load stalls (e.g. slow CDN image)
...
function updateBackToTopState() {
  const scrollTop = window.pageYOffset || document.documentElement.scrollTop;
  backToTop.classList.toggle('back-to-top--visible', scrollTop > 600);
}
